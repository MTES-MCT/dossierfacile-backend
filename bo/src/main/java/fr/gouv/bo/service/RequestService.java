...
public class RequestService {
    ...
    @Async
    public void send(LightAPIInfoModel lightAPIInfoModel, String urlCallback, String partnerApiKeyCallback) {
        ...
        sendRequest(new HttpEntity<>(lightAPIInfoModel, headers), urlCallback);
    }
    ...
    private void sendRequest(HttpEntity request, String urlCallback) {
        ResponseEntity<Object> response;
        try {
            response = restTemplate.exchange(urlCallback, HttpMethod.POST, request, Object.class);
            log.info("CallBack ResponseStatus: {}", response.getStatusCode());
        } catch (RestClientException e) {
            log.error(ACTION_1 + Sentry.captureException(e));
            ...
            if (e instanceof ResourceAccessException) {
                try {
                    ...
                    log.error(ACTION_1 + Sentry.captureException(e1));
