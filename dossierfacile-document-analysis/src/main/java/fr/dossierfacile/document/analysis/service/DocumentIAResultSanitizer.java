...
public class DocumentIAResultSanitizer {
    ...
    private List<GenericProperty> toGenericPropertyList(List<?> rawValues) {
        ...
        for (Object value : rawValues) {
            GenericProperty property = toGenericProperty(value);
            if (property != null) {
    ...
    // Convertit un element brut en GenericProperty en acceptant les objets deja typés
    // et le format mappe issu d'un payload JSON désérialisé.
    private GenericProperty toGenericProperty(Object rawValue) {
        if (rawValue instanceof GenericProperty genericProperty) {
        ...
        property.setName(map.get("name") instanceof String s ? s : null);
        property.setType(map.get("type") instanceof String s ? s : null);
        property.setValue(normalizeRawValue(map.get("value")));
        return property;
    }

    // Normalise recursivement la valeur brute pour conserver la structure
    // attendue par les getters utilitaires de GenericProperty.
    private Object normalizeRawValue(Object rawValue) {
        ...
        for (Object item : listValue) {
            GenericProperty nestedProperty = toGenericProperty(item);
            normalized.add(nestedProperty != null ? nestedProperty : item);
        }
        return normalized;
    }

    // Nettoie les donnees barcode: suppression de rawData.fields et filtrage strict
