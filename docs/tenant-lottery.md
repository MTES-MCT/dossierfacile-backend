...
`OperatorReviewPolicy.isOperatorReviewGranted` (l'état de file, lu par `resolveStatus`) :
- **flag OFF** : comportement historique — `validation_requested = true` accorde la place, donc bloque `COMPLETED` ;
- **flag ON** : `validation_requested` devient purement déclaratif ; seul un ticket **`DRAWN`** accorde la place (un `PENDING` ne bloque pas : le dossier reste utilisable en attendant le tirage).
...
```
 common-library
   ...
   OperatorReviewPolicyImpl       garde « ticket DRAWN » dans isOperatorReviewGranted
...
Garde 409 `canRequestOperatorReview`, persistance de `validation_requested` et logs `VALIDATION_REQUESTED` / `VALIDATION_DECLINED` inchangés.
...
| L9 | Candidat `PENDING` → liaison partenaire DFC | Bascule `TO_PROCESS`, ticket `CANCELLED`, `QUEUE_ENTERED` `PARTNER_LINK` avec `bypass = true`. Filet : un `PENDING` qui survivrait sur un dossier lié est annulé par le tirage suivant, sans cooldown (§6.4) |
| L10 | Candidat `PENDING` → passage en COUPLE | `validation_requested` purgé, ticket `CANCELLED`. Filet : idem L9 pour un dossier non-ALONE (§6.4) |
