...
public class DocumentServiceImpl implements DocumentService {
    ...
    private final PdfSanitizerService pdfSanitizerService;
    ...
    /**
     * OWASP File Upload — "Change the filename to something generated by the application" / "Run the file through CDR".
     */
    private WatermarkDocument createDocument(List<ValidatedFile> validatedFiles, String watermark) {
        List<StorageFile> files = validatedFiles.stream()
                .map(validated -> {
                    MultipartFile multipartFile = validated.file();
                    String detectedMimeType = validated.detectedMimeType();
                    String originalFilename = multipartFile.getOriginalFilename();
                    String safePath = StringUtils.cleanPath(Objects.requireNonNullElse(originalFilename, ""));
                    String ext = FilenameUtils.getExtension(safePath).toLowerCase();
                    String generatedName = UUID.randomUUID() + (ext.isBlank() ? "" : "." + ext);
