...
def run(es, es_index, error_threshold, env, application, start_time, end_time, send_alert):
    ...
    # === Step 2: Aggregate error 4xx counts (excluding 401, 404, 409) per route/method/status ===
    base_query_errors = {
        "bool": {
    ...
    for route_method, status_counts in errors_by_route_status.items():
        ...
        if ratio >= error_threshold and total_requests > 1:
    ...
    if issues:
        ...
        table = build_table(
            ...
            headers=["URI", "Status Code", "Methods", "Total Requests", "4xx Errors", "Error Rate"],
            rows=rows,
        ...
        send_alert(f"High 4xx Error Rate Detected between {start_time_str} and {end_time_str}", alert_msg, "warning")
