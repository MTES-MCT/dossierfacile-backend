def run(es, es_index, error_threshold, env, application, start_time, end_time, send_alert):
    ...
    table_headers = "| Method | Route | Error Rate | 4xx Count | Total Requests |"
    table_divider = "| :--- | :--- | :--- | :--- | :--- |"
    table_rows = [
    ...
    title_text = f"⚠️ High 4xx Error Rate Detected ({len(flagged_routes)} routes affected)"
    ...
    attachments = [
        {
            "fallback": title_text,
            "pretext": f" ⚠️ *High 4xx Error Rate Detected ({len(flagged_routes)} routes affected)*",
            "color": "warning",
