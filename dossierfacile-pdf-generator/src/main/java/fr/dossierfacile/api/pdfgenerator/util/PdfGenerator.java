...
public class PdfGenerator {
    ...
    public static byte[] generatePdf(List<PdfMergeModel> pdfMMIn) {
        List<PdfMergeModel> pdfMMOut = new ArrayList<>();
        for (PdfMergeModel pdfMM : pdfMMIn) {
            if(!pdfMM.getApplyWatermark()) {
                pdfMMOut.add(pdfMM);
            }else{
                InputStream fileIS = pdfMM.getInputStream();
                if (pdfMM.getExtension().equals("pdf")) {
                    PdfGenerator.mergeAndWatermarkPdf(fileIS, pdfMMOut);
                } else {
                    PdfGenerator.mergeAndWatermarkImage(fileIS, pdfMMOut);
                }
            }
        }
        return PdfGenerator.mergePdfModels(pdfMMOut).toByteArray();
    }

    private static void mergeAndWatermarkImage(InputStream imageIS, List<PdfMergeModel> pdfMergeModels) {
        try {
    ...
    private static void mergeAndWatermarkPdf(InputStream pdfIS, List<PdfMergeModel> pdfMergeModels) {
    ...
    private static byte[] watermarkImage(byte[] imageBytes) {
        ...
        try {
            ...
            Resource resource = new ClassPathResource("static/pdf/watermark-layer.png");
    ...
    private static ByteArrayOutputStream mergePdfModels(List<PdfMergeModel> pdfMergeModels) {
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        PDFMergerUtility ut = new PDFMergerUtility();
