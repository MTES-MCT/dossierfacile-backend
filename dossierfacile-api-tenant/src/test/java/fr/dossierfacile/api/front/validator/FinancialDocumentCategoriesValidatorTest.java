...
class FinancialDocumentCategoriesValidatorTest {
    ...
    void validationTest(ValidatorTestParam validatorTestParam) {
        ...
        if (validatorTestParam.expectedError != null) {
            when(validationContext.buildConstraintViolationWithTemplate(validatorTestParam.expectedError)).thenReturn(mockBuilder);
        } else {
        ...
        if (validatorTestParam.expectedError != null) {
            ...
            verify(mockBuilder, times(1)).addConstraintViolation();
        } else {
