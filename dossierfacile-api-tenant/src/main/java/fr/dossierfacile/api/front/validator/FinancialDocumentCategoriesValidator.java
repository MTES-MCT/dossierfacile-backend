...
public class FinancialDocumentCategoriesValidator implements ConstraintValidator<FinancialDocument, IDocumentFinancialForm> {
        ...
        switch (documentFinancialForm.getTypeDocumentFinancial()) {
            ...
            case SCHOLARSHIP, NO_INCOME -> {
                if (documentFinancialForm.getCategoryStep() != null) {
                    constraintValidatorContext.buildConstraintViolationWithTemplate("For document sub category " + documentFinancialForm.getTypeDocumentFinancial().name() + " category step has to be null").addConstraintViolation();
                    return false;
                }
                return true;
            }
            default -> {
                constraintValidatorContext.buildConstraintViolationWithTemplate(documentFinancialForm.getTypeDocumentFinancial().name() + " is not a financial sub category").addConstraintViolation();
                return false;
            }
        }
    }
    ...
    private boolean verifyMandatoryStep(
        ...
        if (categoryStep == null) {
            constraintValidatorContext.buildConstraintViolationWithTemplate("For document sub category " + subCategory.name() + " category step can not be null").addConstraintViolation();
            return false;
        }
        if (!availableStep.contains(categoryStep)) {
            constraintValidatorContext.buildConstraintViolationWithTemplate(categoryStep.name() + "is not valid for document sub category " + subCategory.name()).addConstraintViolation();
            return false;
        }
        return true;
