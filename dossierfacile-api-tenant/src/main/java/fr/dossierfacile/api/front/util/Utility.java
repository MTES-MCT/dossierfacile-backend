...
public class Utility {
    ...
    @Value("${water.marker.text}")
    private String waterMarkerText;
    @Value("${water.marker.font.size}")
    private Integer waterMarkerFontSize;
    @Value("${water.marker.lines}")
    private Integer waterMarkerLines;
    ...
    private static List<ByteArrayOutputStream> generateListOfImagesFromPdfPages(InputStream inputStream) {
        ...
        try {
            ...
            PDResources resourcesFirstPage = document.getPage(0).getResources();
            for (COSName cosName : resourcesFirstPage.getFontNames()) {
                PDFont font = resourcesFirstPage.getFont(cosName);
                log.info("FONT in first page : " + font.getName());
            }
    ...
    public ByteArrayOutputStream addWaterMarker(InputStream inputStream) {
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        try (PDDocument originalDoc = PDDocument.load(inputStream)) {
            createLines(originalDoc);
    ...
    private void createLines(PDDocument originalDoc) {
        float height = originalDoc.getPage(0).getMediaBox().getHeight();
        float width = originalDoc.getPage(0).getMediaBox().getWidth();
        for (int j = 0; j < waterMarkerLines; j++) {
            createLineOverlay(originalDoc, width, height, j * (width / waterMarkerLines), 0);
            createLineOverlay(originalDoc, width, height, 0, j * (height / waterMarkerLines));
        }
    }

    private void createLineOverlay(PDDocument originalDoc, float width, float height, float x, float y) {
        try (final PDDocument overlayDoc = new PDDocument()) {
            ...
            contentStream.setFont(font, waterMarkerFontSize);
            contentStream.setNonStrokingColor(0f);
            double c = Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2));
            double angle = (angle(c, width, height));
            contentStream.transform(Matrix.getRotateInstance(angle, x, y));
            contentStream.beginText();
            contentStream.showText(waterMarkerText + " " + waterMarkerText + " " + waterMarkerText + " " + waterMarkerText + " " + waterMarkerText + " " + waterMarkerText + " " + waterMarkerText + " " + waterMarkerText + " " + waterMarkerText + " " + waterMarkerText + " " + waterMarkerText + " " + waterMarkerText + " " + waterMarkerText + " " + waterMarkerText + " " + waterMarkerText + " " + waterMarkerText + " ");
