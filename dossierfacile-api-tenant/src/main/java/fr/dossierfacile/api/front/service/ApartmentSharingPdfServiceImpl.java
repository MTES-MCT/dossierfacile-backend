...
import fr.dossierfacile.api.front.service.interfaces.ApartmentSharingPdfService;
...
@AllArgsConstructor
@Slf4j
public class ApartmentSharingPdfServiceImpl implements ApartmentSharingPdfService {
    ...
    private static ByteArrayOutputStream addTextHeaderAndTextBodyToTheCopyOfAttachmentsAndClarificationTemplate(List<Tenant> tenantList, String headerSentence, String bodyText) {
    ...
    private static String concatenateTheFullTenantNames(List<Tenant> tenantList) {
    ...
    private static String getSentenceForTenantFromDocumentCategory(Tenant tenant, DocumentCategory documentCategory, int count) {
    ...
    private static String getSentenceForGuarantorFromDocumentCategory(int numberOfGuarantor, TypeGuarantor typeGuarantor, DocumentCategory documentCategory, String tenantFirstName, int count) {
    ...
    private static int calculateTotalIncomingOfGuarantor(Guarantor guarantor) {
    ...
    private static String concatenateTheFirstTenantNames(List<Tenant> tenantList) {
        StringBuilder concatenatedNames = new StringBuilder();
        int lastIndex = tenantList.size() - 1;
        int currentIndex = 0;
        for (Tenant tenant : tenantList) {
    ...
    private static TargetImageData adjustSourceImageToAvailableAreaInTemplate(float sourceWidth, float sourceHeight) {
    ...
    private static ByteArrayOutputStream mergeImageInsideTemplate(byte[] imageBytes, byte[] templateBytes) {

        ByteArrayOutputStream result = new ByteArrayOutputStream();
        try {
            //region Scaling source image to required dimensions inside the available area in pdf template
            BufferedImage sourceImage = ImageIO.read(new ByteArrayInputStream(imageBytes));

            TargetImageData targetImageData = adjustSourceImageToAvailableAreaInTemplate(sourceImage.getWidth(), sourceImage.getHeight());
            Image scaledSourceImage = sourceImage.getScaledInstance((int) targetImageData.getTargetWidth(), (int) targetImageData.getTargetHeight(), Image.SCALE_AREA_AVERAGING);

            BufferedImage finalImage = new BufferedImage((int) targetImageData.getTargetWidth(), (int) targetImageData.getTargetHeight(), BufferedImage.TYPE_INT_ARGB);
            Graphics2D graphics2D = finalImage.createGraphics();
            graphics2D.drawImage(scaledSourceImage, 0, 0, null);
            graphics2D.dispose();
            ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
            ImageIO.write(finalImage, "png", outputStream);
            //endregion

            //region printing image inside the pdf template
            PDDocument document = PDDocument.load(templateBytes);
            PDImageXObject pdImageXObject = PDImageXObject.createFromByteArray(document, outputStream.toByteArray(), "");
            ...
            contentStream.drawImage(pdImageXObject, targetImageData.getHorizontalDisplacement(), targetImageData.getVerticalDisplacement());
            contentStream.close();
    ...
    private static void addDocument(PDFMergerUtility ut, InputStream pdfDocument, List<Integer> indexPagesForDocuments, boolean newCategoryDocument, List<Tenant> tenantList, String headerSentence) {
        List<ByteArrayOutputStream> pagesList = Utility.generateListOfImagesFromPdfPages(pdfDocument);
        ...
        for (ByteArrayOutputStream pageImage : pagesList) {
            ByteArrayOutputStream pdfDocPageWithAttachmentMerged = mergeImageInsideTemplate(pageImage.toByteArray(), templateWithTextsHeader.toByteArray());
            ut.addSource(new ByteArrayInputStream(pdfDocPageWithAttachmentMerged.toByteArray()));
        }

        if (newCategoryDocument) {
            indexPagesForDocuments.add(indexPagesForDocuments.get(indexPagesForDocuments.size() - 1) + pagesList.size());
        } else {
            indexPagesForDocuments.set(indexPagesForDocuments.size() - 1, indexPagesForDocuments.get(indexPagesForDocuments.size() - 1) + pagesList.size());
        }
    }
    ...
    private static String getSentenceByApplicationType(ApplicationType applicationType) {
    ...
    private static void addFirstNamesOfTenantsInTheHeaderOfCurrentIndexPage(int indexPage, PDDocument doc, List<Tenant> tenantList, PDType0Font font) throws IOException {
    ...
    private static void addStaticTextInFirstTemplateOfIndexes(int indexPage, PDDocument doc, PDType0Font font) throws IOException {
    ...
    private static void addContentInFirstRectanguleInFirstTemplateOfIndexes(int indexPage, PDDocument doc, PDType0Font font, ApplicationType applicationType) throws IOException {
        PDPageContentStream contentStream1 = new PDPageContentStream(doc, doc.getPage(indexPage), PDPageContentStream.AppendMode.APPEND, true);
    ...
    private static void addContentInSecondRectanguleInFirstTemplateOfIndexes(int indexPage, PDDocument doc, PDType0Font fontTitle, PDType0Font fontContent, String content) throws IOException {
        PDPageContentStream contentStream1 = new PDPageContentStream(doc, doc.getPage(indexPage), PDPageContentStream.AppendMode.APPEND, true);
        contentStream1.setNonStrokingColor(106 / 255.0F, 106 / 255.0F, 106 / 255.0F);
        contentStream1.beginText();
    ...
    private static void addContentInThirdRectanguleInFirstTemplateOfIndexes(int indexPage, PDDocument doc, PDType0Font fontTitle, PDType0Font fontContent, List<Tenant> tenantList) throws IOException {
        PDPageContentStream contentStream1 = new PDPageContentStream(doc, doc.getPage(indexPage), PDPageContentStream.AppendMode.APPEND, true);
        contentStream1.setNonStrokingColor(106 / 255.0F, 106 / 255.0F, 106 / 255.0F);
        contentStream1.beginText();
                ...
                if (urlPdfDocument != null && !urlPdfDocument.isBlank()) {
                    ...
                    if (swiftObject != null) {
                        ...
                        if (dlPayload.getHttpResponse().getStatus() == HttpStatus.OK.value()) {
                            ...
                            log.warn("Problem when downloading document [" + currentCategory.name() + "]");
                        }
                    } else {
                        throw new DocumentNotFoundException(currentCategory.name(), document.getId());
                    }
                }
            }
    ...
    @Override
    public ByteArrayOutputStream generatePdf(ApartmentSharing apartmentSharing) {
